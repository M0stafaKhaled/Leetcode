...
private:
    ...
    int dfs3(int row, int col, vector<vector<int>>& mat) {
        ...
        if (row == col) {
            ...
            res = max(
                {res, dfs3(row + 1, col + 1, mat), dfs3(row, col + 1, mat)});
        } else {
            res = max({res, dfs3(row + 1, col + 1, mat),
                       dfs3(row, col + 1, mat), dfs3(row - 1, col + 1, mat)});
        }
    ...
    int dfs2(int row, int col, vector<vector<int>>& mat) {
        ...
        if (row == col) {
            ...
            res = max(
                {res, dfs2(row + 1, col + 1, mat), dfs2(row + 1, col, mat)});
        } else {
            res = max({res, dfs2(row + 1, col + 1, mat),
                       dfs2(row + 1, col, mat), dfs2(row + 1, col - 1, mat)});
        }
